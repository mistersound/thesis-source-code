...
var Storage = function(prefix, face, keyChain, certificateName, dPrefix)
{
...
Storage.prototype.onRegisterSuccess = function(prefix) 
{
...
// OnRegistration success
Storage.prototype.onRegisterFailed = function(prefix, reason)
{
...
//send Interest
Storage.prototype.sendInterest = function(name) 
{
  console.log("Prepare to send interest for: " + name.toUri());
    ...
    var stor = prefix.toLowerCase().includes("storage");
...
//onData
Storage.prototype.onData = function(interest, data) 
{
...
//onTimeout
Storage.prototype.onTimeout = function(interest) 
{
...
//onNack
Storage.prototype.onNack = function(interest, nack) 
{
...
//onInterest
 Storage.prototype.onInterest = function(prefix, interest, face, interestFilterId, filter)
{

  var interestName = interest.getName().toUri();
  var words = interestName.split('/');
  var dataName = "/" + words[5] + "/" + words[6] + "/" + words[7] + "/" + words[8];
  ...
  //check if it's a heartbeat interest; then check if the name is not advertised by the storage ignore it
	...
	/*start with th ocaml execution */

		var exec = require('child_process').exec, child;
	
	
		var cmd = "./parser/parser_s.native /" + interestName;
		valide = true;
	...
	child = exec(cmd,
		...
		if(stderr.includes("Accepting")){
			console.log("Accepting heartbeat command");
			valide=true;
		}else{
			console.log("non valid command");
			valide = false;
			return;
		}
		if (error !== null) {
   ...
   if(interestName.includes("storage")){


	//check and refuse already replicated data


	var n =  inArray(dataName, this.replicationList); 
 	if(n == true){

	  console.log("ignoring interest for already replicated data: " + interestName);
          return;
 	}

	  //like '/upec/storage/3/3/upec/fileData.txt/0/9'
	//parse the interest name; if not accepted, do nothing; 
	//this means to call the ocaml parser here;
...
child = exec(cmd,
	...
	/*end ocaml execution */
	  console.log("incoming storage command interest: " + interestName);
	...
	//update heartbeatList in configuration file

	save_list("conf/heartbeat.conf",this.heartbeatList);
	
	//send next replication request
	var repIndex = words[4]-1;
	//console.log(repIndex);
	if((repIndex!=0) && (repIndex>0)){
	  var cmd = "/" + words[1] + "/" + words[2] + "/" + words[3] + "/" + repIndex + "/" + words[5] + "/" + words[6] + "/" + words[7] + "/" + words[8];
	  //console.log(cmd);
	  console.log("send interest for the next replication: " + cmd);
	  var nextName = new Name(cmd);
	  this.sendInterest(nextName);
	}
	
	
	//replicate the data; //send interest for the data --> using repo-ng- ndngetfile /ndn/edu/c/data/hello
	console.log("send interest for the data: " + dataName); 
	var regName = words[5] + "_" + words[6];
		
 	var exec = require('child_process').exec, child;
	
	//var cmd = "ndngetfile " + dataName + " > test.txt";
	var cmd = "ndncatchunks " + dataName + " > store/" + regName;
...
child = exec(cmd,
	...
	//save data name in replicationList and update configuration file
	this.replicationList.push(dataName);
	save_list("conf/replication.conf",this.replicationList);
...
//onInterestResponse
 Storage.prototype.onInterestResponse = function(interest,face)
{
...
//sendHeartbeat
 Storage.prototype.sendHeartbeat = function(name)
{
...
function initiate()
{
  ...
  var storage = new Storage(prefix, face, keyChain,
...
//save the content of a list to a file
function save_list(location, list) {
