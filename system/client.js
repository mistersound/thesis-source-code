/******


NDFS Client 
@author: Junior DONGO

***************/

var readline = require('readline');
var Face = require('.').Face;
var Name = require('.').Name;
var Blob = require('.').Blob;
var Interest = require('.').Interest;
var Data = require('.').Data;

var SafeBag = require('.').SafeBag;
var KeyChain = require('.').KeyChain;
var KeyType = require('.').KeyType;
var ChronoSync2013 = require('.').ChronoSync2013;
var UnixTransport = require('.').UnixTransport;


var DEFAULT_RSA_PUBLIC_KEY_DER = new Buffer([
    
]);

// Use an unencrypted PKCS #8 PrivateKeyInfo.
var DEFAULT_RSA_PRIVATE_KEY_DER = new Buffer([
    
]);




var Client = function(prefix, face, keyChain, certificateName, dPrefix)
{

  this.face = face;
  this.keyChain = keyChain;
  this.certificateName = certificateName;
  this.name_prefix = (new Name(prefix));//.append("testApp").append();
  this.dataPrefix  = (new Name(dPrefix));
  this.face.registerPrefix(this.dataPrefix, this.onInterest.bind(this), this.onRegisterFailed.bind(this), this.onRegisterSuccess.bind(this));
  this.sendInterest(this.name_prefix);
    
};


// OnRegistration success
Client.prototype.onRegisterSuccess = function(prefix) 
{
  console.log("Prefix registration succeeded for: " + prefix);

};

// OnRegistration success
Client.prototype.onRegisterFailed = function(prefix, reason)
{
  console.log("Prefix registration failed for: " + prefix + " for this reason: " + reason);
  face.close();  // This will cause the script to quit.
};



//send Interest
Client.prototype.sendInterest = function(name) 
{
  console.log("Prepare to send interest for: " + name.toUri());
  
    var inter = new Interest();
    inter.setName(name);
    inter.setInterestLifetimeMilliseconds(1000);
    var prefix = name.toUri();

    var stor = prefix.toLowerCase().includes("storage");
    var ht = prefix.toLowerCase().includes("heartbeat");
    var st = prefix.toLowerCase().includes("stop"); 
    if(stor||ht||st){
	inter.setMustBeFresh(true);    
    }
    
    this.face.expressInterest(inter, this.onData.bind(this), this.onTimeout.bind(this), this.onNack.bind(this));
    
  
}



//onData
Client.prototype.onData = function(interest, data) 
{
  console.log("Received " + data.getName().toUri());
  console.log(data.getContent().buf().toString('binary'));
  //face.close();
}

//onTimeout
Client.prototype.onTimeout = function(interest) 
{
  console.log("Time out for interest " + interest.getName().toUri());
  face.close();  // This will cause the script to quit.
};


//onNack
Client.prototype.onNack = function(interest, nack) 
{
  console.log("Nack for interest " + interest.getName().toUri() + " for the reason : " + nack.getReason());

};



//onInterest
 Client.prototype.onInterest = function(prefix, interest, face, interestFilterId, filter)
{


  //console.log(interest.getName().toUri());
  var interestName = interest.getName().toUri();

  console.log("Interest received for: " + interestName);

 //console.log(this.name_prefix.toUri());


  var prefixData = this.dataPrefix.toUri(); //"/upec/fileData.txt"
  
  var prefixStop = prefixData.concat("/stop");

  //Client doesn't have to respond to heartbeat Interest
    if(interestName.includes("heartbeat")){
 
   	 console.log("ignoring heartbeat interest: " + interestName);
 
   	 return; 
    }


    if (interestName.includes("stop")) {
 
       console.log("interest is a stop command request");

       this.onInterestResponse(interest,this.face);   //respond to the stop interest this is a kind of acknowledgment before the stop of the process
 	/*
       //for the moment send the interest for the sync of the global list;
       std::string syn = m_prefix_sync.toUri() + "/rep" + m_prefix_data.toUri();
       std::cout << "sync : " << syn << std::endl;
 
       Name sync_Name = Name(syn);
       sendInterest(syn);
	*/            

       console.log("replication process is over; data replicated correctly");
        //stop the process 
        face.close();
        return;
    }else{
	console.log("interest is a data request");
        }
    /* else {
        //autre/chose
        console.log("ignoring unrecognized interest");
 
        return ;
    }*/
        this.onInterestResponse(interest,this.face);



  

};

//onInterestResponse
 Client.prototype.onInterestResponse = function(interest,face)
{
  console.log("Sending response for: " + interest.getName());

  var content = "HELLO";

  var dataName = new Name(interest.getName());
 

  if (content != null) {
  
    var co = new Data(interest.getName());
    co.setContent(content);
    console.log("Send data for: " + interest.getName());
    this.keyChain.sign(co, this.certificateName, function() {

      try {
        face.putData(co);
      }
      catch (e) {
        console.log(e.toString());
      }
    });
  }

};



function initiate()
{


  // Gives the posibility to type the command
 var rl = readline.createInterface({
  input: process.stdin,
  output: process.stdout
 });
 

 rl.question("Enter the command for replication: ", function(word) {
 

  var prefix = word;
 

 var dPrefix = "/lacl/data/0/9/stop"; //"/upec/test";
  
  var face = new Face(new UnixTransport());

  // Set up the KeyChain.
  var keyChain = new KeyChain("pib-memory:", "tpm-memory:");
  // This puts the public key in the pibImpl used by the SelfVerifyPolicyManager.
  keyChain.importSafeBag(new SafeBag
    (new Name("/testname/KEY/123"),
     new Blob(DEFAULT_RSA_PRIVATE_KEY_DER, false),
     new Blob(DEFAULT_RSA_PUBLIC_KEY_DER, false)));

  face.setCommandSigningInfo(keyChain, keyChain.getDefaultCertificateName());
  
  var client = new Client(prefix, face, keyChain,
     keyChain.getDefaultCertificateName(),dPrefix);


 rl.close();

 });

 
}

initiate();
